...
function pbkdf2HmacSha256 (P, S, c, dkLen) {
  if (typeof P === 'string') P = new TextEncoder().encode(P) // encode pw as UTF-8
  else if (!ArrayBuffer.isView(P)) P = new Uint8Array(P)

  if (typeof S === 'string') S = new TextEncoder().encode(S) // encode S as UTF-8
  else if (!ArrayBuffer.isView(S)) S = new Uint8Array(S)
...
function scryptBlockMix (B) {
  ...
  */
  ...
  3.  B' = (Y[0], Y[2], ..., Y[2 * r - 2],
            Y[1], Y[3], ..., Y[2 * r - 1])
  */
  const Bprime = new BigUint64Array(B.length)
  let odd = false
  for (let i = 0; i < 2 * r; i++) {
    const offset = i * 8
...
function scryptROMix (B, N) {
  ...
  */
  ...
  function integerifyModN (bigUint64arr) {
    ...
    // Since N is a power of and N <= 2**64 I can just take the last subblock (little endian) of 64 bits
    return lastBlock.getBigUint64(0, true) % Nbi
  }
  for (let i = 0; i < N; i++) {
    const j = integerifyModN(X)
    typedArrayXor(X, V[j])
    X = scryptBlockMix(X)
  }
  return X
...
async function scrypt (P, S, N, r, p, dkLen) {
  ...
  return require('crypto').scryptSync(P, S, dkLen, { N, r, p })
}
